...
describe('网易云例子', () => {
    it('点击按钮后，count的值会+1处理', () => {
        ...
        // 断言
        expect(Number(vm.$el.querySelector('span').textContent)).toBe(1)
    })
    ...
    it('获取元素并断言值是否一致', () => {
        ...
        expect(h3.text()).toBe('counter.vue')
    })
})
...
describe('Vue Test Utils官方例子', () => {
    ...
    it('从子组件触发事件', async () => {
        await wrapper.findComponent(CounterChild).vm.$emit('custom')
        expect(wrapper.html()).toContain('Emitted!')
    })

    it('仿造Prop', () => {
        expect(wrapper.vm.foo).toBe('some value')
    })
