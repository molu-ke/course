(function (root) {
    ...
    Data.prototype = {
        key: function (elem) {
            ...
            if (!unlock) {
                unlock = Data.uid++ // 1
                descriptor[this.expando] = { // 钥匙  descriptor.jQuery102093881699983413220.37598373945761576 = { value:1 } }
                    value: unlock
                }

                // 方法直接在一个对象上定义一个或多个新的属性或修改现有属性，并返回该对象  赋值操作
                // elem.jQuery102093881699983413220.37598373945761576 = 1
                Object.defineProperties(elem, descriptor)
            }

            // 创建对应的缓存对象(数据仓库) this.cache[1] = {}
        ...
        set: function (owner, key, value) {
            ...
            if (typeof key === 'string') { // firstqueue
                cache[key] = value // cache.firstqueue = [ function(){} ]
            }
        ...
        remove: function (owner, key) {
            // console.log(owner, key)  // elem   [firstqueue  firstqueueHooks]
            ...
            if (elem) {
                ...
                if (data) {
                    if (!queue || jQuery.isArray(data)) {
                        queue = data_priv.access(elem, type, jQuery.markArray(data)) // [function(){}]
                    } else {
                        queue.push(data)
                    }
                }
